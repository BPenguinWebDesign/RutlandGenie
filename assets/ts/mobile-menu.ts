...
// Get container
const fadeOuts = Array.from(document.getElementsByClassName('fade-out-menu')).map(v => ({ elem: v, transition: (v as HTMLElement).style.transition })) as {elem: HTMLElement, transition: string}[];

const elem = document.getElementsByClassName('container')[0]! as HTMLElement;
